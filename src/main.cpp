    ...
    if(selector::autonColor == 0){
		switch(selector::autonType){ //Red
			case 0://Close
				...
				Intake.move(127);
				Indexer.move(-127);
				chassis.moveToPose(-36,24,90,3000,{.maxSpeed = moveSpeed,.minSpeed = moveSpeed*0.5f},false);
				chassis.moveToPoint(-13,24,3000,{.maxSpeed = moveSpeed*0.5f},false);//pick up center cubes
				chassis.turnToPoint(-24,24,3000,{.maxSpeed = turnSpeed},false);
				chassis.moveToPoint(-24,24,3000,{.maxSpeed = moveSpeed},false);
				chassis.turnToPoint(0,0,3000,{.maxSpeed = turnSpeed},false);
				chassis.moveToPoint(-18,18,2000,{.maxSpeed = moveSpeed},false);
				Intake.move(127);
				Indexer.move(70);
				Upper.move(-127);
				delay(3000);
				...
				Indexer.brake();
				Intake.brake();
				chassis.moveToPoint(-48,48,3000,{.forwards = false,.maxSpeed = moveSpeed},false);
				Tongue.set_value(true);//extend tongue
				chassis.turnToPoint(-58,48,3000,{.maxSpeed = (int)moveSpeed},false);
				Intake.move(127);
				Indexer.move(-127);
				//chassis.moveToPoint(-35,48,2000,{.forwards = false,.maxSpeed = moveSpeed},false);
				chassis.moveToPose(-60,48,-90,2000,{.maxSpeed = moveSpeed},false);//move to tube
				...
				chassis.moveToPose(-30,49,90,2000,{.maxSpeed = moveSpeed},false);//move to long goal
				Intake.move(127);
				Indexer.move(127);
			...
			case 1://Far
				...
				chassis.moveToPoint(-13,-24,3000,{.maxSpeed = moveSpeed*0.5f},false);//pick up center cubes
				//chassis.turnToPoint(-24,-24,3000,{.maxSpeed = turnSpeed},false);
				chassis.moveToPoint(-24,-24,3000,{.forwards = false,.maxSpeed = moveSpeed},false);
				chassis.turnToPoint(0,0,3000,{.maxSpeed = turnSpeed},false);
				chassis.moveToPose(-14,-14,45,2000,{.maxSpeed = moveSpeed,.minSpeed = moveSpeed*0.75},false);
				Intake.move(-127);
				Indexer.move(127);
				delay(2000);
				//chassis.moveToPose(-12,-12,45,2000,{.maxSpeed = moveSpeed},false);
				//chassis.turnToPoint(-48,-48,3000,{.maxSpeed = turnSpeed},false);
				Indexer.brake();
				Intake.brake();
				chassis.moveToPoint(-48,-48,3000,{.forwards = false,.maxSpeed = moveSpeed},false);
				...
				Indexer.move(-127);
				chassis.moveToPoint(-35,-48,2000,{.forwards = false,.maxSpeed = moveSpeed},false);
				chassis.moveToPose(-60,-48,-90,2000,{.maxSpeed = 110,.minSpeed = 110},false);//move to tube
				delay(3000);//wait a couple of seconds to intake
				chassis.moveToPoint(-48,-48,2000,{.forwards = false,.maxSpeed = moveSpeed},false);//reverse out of the tube
				chassis.turnToHeading(90,2000,{.maxSpeed = turnSpeed},false);
				Tongue.set_value(false);//retract tongue
				//delay(500);//half a second delay to let tongue fully retract
				chassis.moveToPose(-30,-47,90,2000,{.maxSpeed = moveSpeed},false);//move to long goal
				Intake.move(127);
				Indexer.move(127);
				Upper.move(127);//outake
				delay(5000);
			...
			case 2://Anti Auton
				...
				chassis.moveToPoint(0,10,2000,{.maxSpeed = moveSpeed},false);
				break;
		}
		...
		switch(selector::autonType){
			case 0://Skills 1
				...
				for(int i=0;i<5;i++){//rock back and forth to intake balls better
					...
					chassis.moveToPoint(53,47,1000,{.forwards = false,.maxSpeed = moveSpeed},false);
					chassis.moveToPose(58,47,90,1000,{.maxSpeed = moveSpeed},false);
				}
...
void opcontrol(){
	...
	while (true){
		...
		if(Controller1.get_digital(E_CONTROLLER_DIGITAL_L2)){//intake
			intake = 1.0;
			upper = 0.0;
			indexer = -1.5;
		}
		if(Controller1.get_digital(E_CONTROLLER_DIGITAL_DOWN)){//outake low
			intake = -1.0;
			indexer = 1;
			upper = 0.0;
		}
		if(Controller1.get_digital(E_CONTROLLER_DIGITAL_L1)){//outake middle
			upper = -1.0;
			indexer = 1.0;
			intake = 1.0;
		}
		if(
			Controller1.get_digital_new_press(E_CONTROLLER_DIGITAL_R1)||
			Controller1.get_digital_new_press(E_CONTROLLER_DIGITAL_UP)
		){//outake high
			upper = 1.0;
			indexer = 1.0;
			intake = 1.0;
		...
		if(
			Controller1.get_digital_new_press(E_CONTROLLER_DIGITAL_R2)||
			Controller1.get_digital_new_press(E_CONTROLLER_DIGITAL_A)
			//Controller1.get_digital_new_press(E_CONTROLLER_DIGITAL_Y)
		){//full reset
			upper = 0.0;
			indexer = 0.0;
			intake = 0.0;
		...
		if(Controller1.get_digital(E_CONTROLLER_DIGITAL_B)){//reverse indexer
			indexer = -1.0;
		}
		if(Controller1.get_digital_new_press(E_CONTROLLER_DIGITAL_RIGHT)){//tongue togle
			pistonUpdate();
			intake = 1.0;
			upper = 0.0;
			indexer = -1.5;
